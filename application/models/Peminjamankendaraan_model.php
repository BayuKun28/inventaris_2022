...
class Peminjamankendaraan_model extends CI_Model
{
    public function read()
    {
        $query = "SELECT p.id,p.nama,
                        ...
                        WHEN j.id = 1 THEN (
                                                ...
                                                FROM kendaraan k
                                                WHERE k.id = p.nama_unit
                        )
                        WHEN j.id = 2 THEN (
                                                SELECT e.nama_barang
                                                FROM elektronik e
                                                WHERE e.id = p.nama_unit
                        )
                        END) as nama_unit,p.nama_unit as idunit,
                        p.tgl_pinjam,
                        p.tgl_kembali,
                        p.keterangan
                FROM peminjaman p
    ...
    public function detail($id)
    {
        $query = "SELECT p.id,p.nama as nama_peminjam,
                        ...
                        END) as nama_unit,p.nama_unit as idunit,k.merk,k.tipe_ranmor,u.unit_pengguna,k.no_pol,k.no_rangka,k.no_mesin,
                ...
                FROM peminjaman p
                    LEFT JOIN jenis j on j.id = p.jenis_pinjam
                    LEFT JOIN kendaraan k on k.id = p.nama_unit
